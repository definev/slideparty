...
class OnlineBubblesPlayboard extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return state.state.map(
      waiting: cannotHappenState,
      connected: cannotHappenState,
      wrongBoardSize: cannotHappenState,
      roomData: (data) {
          ...
          child: Center(
            child: Column(
              ...
              children: [
                ConstrainedBox(
                  constraints:
                      const BoxConstraints(maxWidth: 425, maxHeight: 425),
                  child: LayoutBuilder(
                    builder: (context, constraints) {
                      switch (AppInfos.screenType) {
                        case ScreenTypes.touchscreen:
                          return SwipeDetector(
                            onSwipeLeft: () => controller
                                .moveByGesture(PlayboardDirection.left),
                            onSwipeRight: () => controller
                                .moveByGesture(PlayboardDirection.right),
                            onSwipeUp: () =>
                                controller.moveByGesture(PlayboardDirection.up),
                            onSwipeDown: () => controller
                                .moveByGesture(PlayboardDirection.down),
                            child: PlayboardView(
